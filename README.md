...
The <i>src</i> subfolder of the <i>python</i> folder contains am example script <i>subscribe.py</i> that uses the Python API to subscribe to data without directly exposing the JSON websockets API. In the <i>src</i> subfolder of the <i>json</i> folder there is a similarly named example Python script that uses the JSON Websocket API directly and only requires the <a href=https://pypi.org/project/requests/>requests</a> package as a dependency.  
